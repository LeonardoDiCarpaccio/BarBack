...
var Assistant = {
    ...
    get : function (req, callback) {
        ...
        if (isDef(body)) {
            ...
            if (table.length > 0)
                return db.query('SELECT ' + target + ' FROM assistant_carte WHERE ' + table.join(" AND "), callback);
            else
                return db.query('SELECT ' + target + ' FROM assistant_carte', callback);
        } else {
    ...
    update: function (req, callback) {
        ...
        if (isDef(body.id)) {
            ...
            if (table.length > 0) {
                let query = "UPDATE assistant_carte SET " + table.join(',') + " WHERE id =" + body.id
    ...
    delete: function (req, callback) {
        ...
        if (table.length > 0) return db.query("DELETE FROM assistant_carte WHERE " + table.join(" AND "), callback)
        else return callback("ERROR PARAMETERS")
    }
}
